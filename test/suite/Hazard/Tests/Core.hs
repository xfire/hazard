...
normalContentIsCopied = testCase
    "normal files and directories are copied from the input to the output directory" $
        withSystemTempDirectory "hazardTests." $ \outdir -> do
...
specialContentIsNotCopied = testCase
    "special files and directories beginning with an underscore are not copied to the output directory" $
        withSystemTempDirectory "hazardTests." $ \outdir -> do
